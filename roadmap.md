...
Implement embedText(partialText)
Write tests to embed strings
Provide output shape & type to DB engineer

Implement the local embedding function that processes raw user text securely inside our server.
...
- Create a small test harness that prints sample embeddings.
...
Test retrieval with fake embeddings
...
- Hugging Face Inference API (Embeddings model, e.g. sentence-transformers or all-MiniLM)
- node-postgres
- pgvector extension
